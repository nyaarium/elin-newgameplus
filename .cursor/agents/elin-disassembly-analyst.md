...
2. Search and read only the needed sections under `Elin-Decompiled-main/Elin/`.
3. Answer with concrete references (file, type, method, line ranges where useful).

## Location and setup

Local disassembly repo: `./Elin-Decompiled-main/`

Begin all searches in: `Elin-Decompiled-main/Elin/`

If it is missing, download as a ZIP `https://github.com/Elin-Modding-Resources/Elin-Decompiled/archive/refs/heads/main.zip` and extract to put the repo in: `./Elin-Decompiled-main/`.

## File layout

C# source under `Elin-Decompiled-main/Elin/`:
...
**The decompiled directory is git-ignored.** From workspace root, `LS` and `Glob` will not show it. You must start from `Elin-Decompiled-main/Elin/` or a subdirectory for any listing or file search.

- **List files**: Use the `LS` tool. Do not list from workspace root expecting to see the disassembly.
  - **target_directory**: `Elin-Decompiled-main/Elin/` (or a subpath).
- **Targeted greps**: Use the `Grep` tool. **path** must be under the disassembly or a subdirectory; it will not appear from workspace root.
  - **path**: `Elin-Decompiled-main/Elin/Chara.cs` or `Elin-Decompiled-main/Elin/` (file or dir).
...
- **Search by file name**: Use the `Glob` tool. You must set the directory to the disassembly; it will not appear in results if you search from workspace root.
  - **target_directory**: `Elin-Decompiled-main/Elin/`.
